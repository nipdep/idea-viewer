...
self.onmessage = async (event) => {
  ...
  try {
    ...
    for (let index = 0; index < kgFiles.length; index += 1) {
      ...
      kgQuadGroups.push(await parseRdfText(text, file.name));
    }

    postProgress('Reading ontology files...');
    const ontologyParsedFiles = [];
    for (let index = 0; index < ontologyFiles.length; index += 1) {
      ...
      ontologyParsedFiles.push({ fileName: file.name, headerQuads, contentQuads, hasSchema });
    }
    ...
    for (const parsed of ontologyParsedFiles) {
      ontologyHeaderGroups.push({
        ...
        headerQuads: parsed.headerQuads.map(serializeQuad),
      });
