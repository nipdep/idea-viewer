const C = { visibleRadius: 10, minHeadroom: 12, headroomScale: 38, edgeGap: 18, siblingGap: 8, rotations: 16, componentGap: 80, futureDiscount: .58, edgeTolerance: .001 };
const TAU = Math.PI * 2;
...
function candidateFan(center, children, planted, index) {
  ...
  const slots = Array.from({ length: degree - (center.parent ? 1 : 0) }, (_, i) => parentAngle + (center.parent ? i + 1 : i) * TAU / degree);
  const ordered = [...children].sort((a, b) => b.demand - a.demand || b.rank - a.rank || a.id.localeCompare(b.id)); let best = null;
...
export function applyRankAwareRadialBfsLayout(cy) {
  ...
  return { stable: treeError <= C.edgeTolerance, maxDisplacement: 0, collisions: 0, hopViolations: 0 };
}
